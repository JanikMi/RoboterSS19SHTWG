...
WallFollowing::WallFollowing(ros::NodeHandle& nodeHandle): nodeHandle_(nodeHandle)
{
  int queue_size;
  std::string topic;
  ...
  ros::Subscriber subscriber = nodeHandle.subscribe(topic, queue_size, &WallFollowing::chatterCallback, this);
  odom_subscriber_ = nodeHandle.subscribe("odom", queue_size, &WallFollowing::odomCallback, this);
...
bool WallFollowing::readParameters()
{
}

void WallFollowing::topicCallback(const sensor_msgs::Temperature& message)
{
  //algorithm_.addData(message.temperature);
}

void WallFollowing::odomCallback(const nav_msgs::Odometry odomFrame)
{
  globalOdomFrame = odomFrame;
}

void WallFollowing::chatterCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
